...
import java.{util => ju}
import scala.collection.JavaConverters.{asJavaIterableConverter, asScalaSetConverter, iterableAsScalaIterableConverter}
...
object ScalaJavaConversions extends App {
  ...
  println(backToScala eq numbers) // false - not referentially correct. different instances
  println(backToScala == numbers) // true

//  juNumbers.add(7) // unsupported operation exception

  /*
    Exercise

    scala -> java conversion from Optional -> Option
   */
