...
object MagnetPattern extends App {
  ...
  trait Actor {
    ...
    def receive(responses: P2PResponse): Int
    def receive[T : Serializer](message: T): Int
    def receive[T: Serializer](message: T, statusCode: Int): Int
    def receive(future: Future[P2PRequest]): Int
//    def receive(future: Future[P2PResponse]): Int -> type erasure. fails to compile
  }
  ...
  /*
    ...
    3. code duplication
    4. default arguments and type inference
      actor.receive(??) -> compiler doesn't know what default args to use for each method
   */
