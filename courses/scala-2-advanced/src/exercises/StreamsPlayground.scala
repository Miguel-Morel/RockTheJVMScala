...
/*
  Exercise
  implement a lazily evaluated, singly linked stream of elements (stream = head available but tail lazily evaluated)
...
class Cons[+A](hd: A, tl: => MyStream[A]) extends MyStream[A] {
  ...
  /*
    ...
    mapped = s.map(_ + 1) = new Cons(2, s.tail.map(_ + 1))
      (at some point) -> ... mapped.tail <-- forces evaluation of s.tail.map(_ + 1)
   */
  ...
  def filter(predicate: A => Boolean): MyStream[A] = // preserves lazy evaluation
    if (predicate(head)) new Cons(head, tail.filter(predicate))
    else tail.filter(predicate)

  def take(n: Int): MyStream[A] = { // takes the first n elements out of this stream. preserves lazy evaluation
    if (n <= 0) EmptyStream
...
object StreamsPlayground extends App {
  ...
  startFrom0.take(10000).foreach(println) // call by name + lazy vals (call by need) keep this from stack overflow
  ...
  //2.
  /*
    eratosthenes' sieve idea: given a stream of integers starting from 2, the head is always prime. You then eliminate all multiples of that prime from the tail, and recurse.
