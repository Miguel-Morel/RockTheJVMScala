...
// generates a given (implicit):  configReader: ConfigReader[EmberConfig]
...
object EmberConfig {
  ...
  given hostReader: ConfigReader[Host] = ConfigReader[String].emap { hostString =>
    Host
      ...
      .toRight(CannotConvert(hostString.toString, Host.getClass.toString, s"invalid host string: $hostString"))
  }

  given portReader: ConfigReader[Port] = ConfigReader[Int].emap { portInt =>
    Port
      .fromInt(portInt)
      .toRight(CannotConvert(portInt.toString, Port.getClass.toString, s"invalid port number: $portInt")) // equivalent to match above on hostReader
  }
