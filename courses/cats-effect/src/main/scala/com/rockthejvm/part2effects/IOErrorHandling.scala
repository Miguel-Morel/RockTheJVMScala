...
object IOErrorHandling {
  ...
  val aFailure: IO[Int] = IO.raiseError(new RuntimeException("a proper fail")) // more readable than above
  ...
  // redeem: transform the failure and the successs in one go
  ...
  // exercises (all 3 below implemented as fromOption, fromTry, fromEither)
