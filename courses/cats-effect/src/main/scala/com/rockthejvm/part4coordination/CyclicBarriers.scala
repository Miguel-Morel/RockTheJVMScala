 ...
 */
  ...
  // example: signing up for a social network that is just about to be launched
  def createUser(id: Int, barrier: CBarrier): IO[Unit] = for {
    ...
    _ <- IO(s"[user $id] wow, this is so cool").debug
  } yield ()

  def openNetwork(): IO[Unit] = for {
    _ <- IO("[announcer] the rockthejvm social network is up for registration. launching when we have 10 users").debug
    barrier <- CBarrier(10)
