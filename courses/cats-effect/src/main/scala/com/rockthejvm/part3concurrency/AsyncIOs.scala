...
object AsyncIOs extends IOApp.Simple {
  ...
  // async is FFI (foreign function interface) = method to bring the result of a computation performed by some other entity into CE
  ...
  def demoAsyncCancellation() = {
    val asyncMeaningOfLife_v2: IO[Int] = IO.async { cb: Callback[Int] =>
      /*
        finalizer in case computation gets canceled
        finalizers are of type IO[Unit]
        not specifying a finalizer means we need => Option[IO[Unit]]
